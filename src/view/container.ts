...
let DEV = false // true = compile; false = compile only if storage not found
...
export class Container {
    ...
    hydrateUI() {
        ...
        let nodes = (DEV === true) ? null : JSON.parse(localStorage.getItem('elementDescriptors'));
        ...
        for (let index = 0; index < nodes.length; index++) {
            let node = nodes[index] as ElementDescriptor
                // hydrate the node 
                switch (node.kind) {
                    case "label":
                        label(node)
        ...
        let surface = document.getElementById("surface").parentElement!
